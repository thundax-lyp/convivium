...
3. Convivium 支持的最低 DSH 版本必须为 `0.1.1-rc.1`；低于该版本时插件必须拒绝加载并说明版本要求，不得以缺失生命周期能力的降级模式运行会议。
...
2. 用户必须能够暂停、恢复、结束会议，以及在有权限时撤销或重新分配当前发言权。
3. 会议运行时，面板必须显示“暂停”；会议已暂停时，面板必须显示“继续”，并清楚显示暂停原因和发起者。
4. 任何降级选择、强制结束、审核豁免、风险接受和部分完成都必须向用户显示原因。
5. 产品必须通过完整且经过授权的会议状态读取展示正式会议事实，不得把本地缓存或自然语言摘要当作状态真相源。
6. 用户重新打开或刷新会议后，必须看到完整且一致的当前事实。
7. 会议操作可以出现在 DSH 原生工具调用记录中，但这些记录不得替代正式会议状态、transcript 或审计记录。
