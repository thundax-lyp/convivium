...
状态：已确认的目标业务流程；2026-09-15 用户逐项确认轮次、证据包、角色分工、可见性、审核上下文、补充上限与响应期限，并授权补齐非核心收口细节。本文定义目标业务验收，不声明实现覆盖。
...
- 不由 Manager 判断证据内容的真实性、可信度或对观点的支撑度；Manager 的提交要素确认不是证据审核。
...
2. 被拒绝者不参与本轮对话，Manager 向本人返回拒绝信息，会议不记录该次举手，也不因其未回复而等待轮次收口。Manager 可以依据当前议题安排人选，但不得用该安排预判尚未提交证据的内容真伪。
...
1. 一位贡献者在一轮中最多拥有一份会议证据包，使用稳定标识和当前版本。首份提交、格式补正和后续实质补充均作用于该证据包，不新增第二份会议证据。贡献者自己保留历次提交记录；会议只展示和引用该人该轮的一份当前有效证据包，旧审核须能追溯到所针对的版本。
...
1. Manager 只核对说明文件和参考资料清单的格式、必备要素、引用定位与可访问性说明；可以接纳、要求补正或暂缓登记。Manager 不能质疑材料内容是否真实、观察是否可信、观点是否被充分支撑，或改写贡献者提交的内容。要素未知但明确标记时不得要求编造确定值；“暂缓登记”只使该次提交继续处于要素补正状态，不把已接纳举手当作初次举手遭拒或已完成轮次。
2. 初次证据包通过要素确认后登记进入会议。作者、Manager、证据审核员和会议可处理该轮证据；本轮其他贡献者在轮次结束前不能读取该证据包或其审核意见。因此 B 的本轮准备及审核不能交叉引用 A 本轮的证据。
...
3. 正常补充只在当前版本审核完成、意见发送完成、贡献者再举手并获 Manager 接纳后进行。审核进行中不修改该证据包；新版本须按相同固定基线重新审核，旧评分不沿用。
...
1. 正常收口要求每个已接纳举手都有确定状态：未提交者已提交证据或明确退出／按适用期限记录未提交退出；已提交者已明确放弃／被拒继续补充／超时退出；所有已登记证据的最终版本均完成审核。Manager 确认这些贡献者状态，证据审核员确认最终审核状态，会议才收口并公开本轮证据与审核意见。初次举手遭拒且未获接纳的人不计入收口等待对象。若本轮没有已接纳举手，会议可收口空轮次并由 Manager 重新规划；空轮次不生成证据或完成事实。
...
3. 一人一轮的首份证据、格式补正及两次实质补充只产生一份当前有效证据包；首份与格式补正不消耗机会，第三次实质补充申请被拒，新版本重新审核且旧评分不冒充新评分。
4. Manager 能指出缺失的说明文件或资料清单要素，不能据内容可信度或观点支撑度退回；审核员对已登记版本分别给出来源、可信度、完整性及具体观点支撑度的数值或无法判断与理由，不因来源疑点删除登记事实。
