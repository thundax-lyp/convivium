...
需要先校准原生部署资源定位和完整 config 组合契约，再修改随包 patch、wrapper、对应测试及操作说明。当前 T5 不允许修改这些实现，故在此 STOP；没有扩大权限、复制 Preset、注入假 Skill 或更换 provider 继续测试。
...
长期模型任务质量、独占 Skill/per-child Preset、动态 admission、日常 profile 和 Host capability 内容变更后的历史快照继续不在本次范围。当前结论是角色模型与静态资源已实现、真实部署阻塞，不能称为部署完善。

### Native Probe Repairs And Current Network Blocker
