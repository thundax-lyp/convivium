...
2026-09-07，`bd0159b` 后的历史分层调整在 DSH `0.1.1-rc.2`、web profile/spawn provider 上运行：`smoke:profile --all` 14/14 PASS（97.262 秒）、默认五场景 5/5 PASS（36.050 秒），均一次构建并逐场景 Restore；安装与 Host 启动失败注入均无 PASS 或临时根残留，`reassign` Browser mode 只核对 ready 与停止清理。同期 `pnpm --dir plugin verify` 为 76 文件/664 用例及后续 gate PASS，单场景 `--json` 可解析归档 DTO。三种重复收敛 selector 被删除，规则差异交由领域测试；下节保存它们的旧真实运行摘要。上述执行过程及参数清理详情可从 Git 历史追溯，不证明当前入口、人工页面交互、真实模型、跨 Host 或长期压力。
