...
| FR-9 暂停、恢复与故障隔离 | 部分实现 | Captain tool 与 loopback Web pause/resume、outbox guard、SQLite recovery、archive recovery、stale gate 和连续两轮本地 pause/resume 回归已实现 | `speakerTimeoutMs` 尚未接入 Runtime；发言改派工具、attempt failure counter、真实 cold restart/rebind smoke 未完成 |
...
| FR-11 可观察性与用户控制  | 部分实现 | caller-specific status、loopback list/status/pause/resume HTTP、Plugin Client Meetings slot、poll/refetch 和真实浏览器选择/暂停/恢复已实现 | 结构化 metrics、远程/多用户控制、end/reassign 用户控制不在当前闭环                                                |
