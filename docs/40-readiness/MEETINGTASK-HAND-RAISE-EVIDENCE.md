...
状态：已完成自动化与独立 DSH profile 验证（2026-08-27）

## 自动化验证

- `plugin/` 执行 `pnpm verify` 通过：30 个测试文件、193 个测试通过；format、lint、host/client typecheck、build、DSH environment、plugin contract 和 package verification 全部通过。
...
结果：通过。`web` profile 使用 `spawn` provider，真实探针观察到 3 个 Participant continuable Session 的 FIFO transcript：`participant-a → participant-c → participant-b`，并观察到下一轮 Manager plan 与首个 SpeakerAttempt。
