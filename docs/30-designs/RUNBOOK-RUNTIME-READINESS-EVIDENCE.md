# RUNBOOK：最终 HEAD Runtime Readiness Evidence

状态：`Executable`；当前 A/B 产品实现尚未合入，因此现在执行 G0 必须 STOP
...
1. **现在能否执行 evidence/readiness：不能。** A/B RUNBOOK 为 `Executable` 只证明实现说明决策完备，不证明产品代码已实现、测试已通过或已合入。当前执行 G0 必须因 B/A postcondition 不在最终 `origin/main` 而 STOP；不得运行 evidence 或写 readiness。
2. **B 合入、A rebase 后合入且两线 PASS 后能否直接启动：能。** 从 G0 开始执行，无需用户补充产品、架构、接口或验证判断。
3. **是否仍须固定 merged HEAD：必须。** Investigate→Author→Audit 是 G0 的内置机械阶段，不是外部阻塞：G0 从 `origin/main` 取得 40-char SHA，创建唯一 evidence branch，只把上方 `UNSET` 替换为该 literal，检查本文预先固定的 paths/symbols/tests/selectors 与 target 一致并完成 Audit。该步骤不需要用户决定；任何不一致是执行期机械 STOP。

最终判定：`现在不可执行；A/B 合并后可从 G0 直接机械执行，无其他前置阻塞`。
...
B D1-D5 merged
  ...
  -> G0 pins literal origin/main SHA and audits the target
...
- `convergence` 只按 A 的四个固定 assertion 取证。
...
| real profile runner | `plugin/scripts/smoke-profile.mjs`；B 先加一 branch，A 后加一 branch | G0/G3，只运行  |
...
- `plugin/scripts/smoke-profile.mjs` 的 selector `decision-risk-closure`
...
- B focused files：`plugin/tests/unit/protocol/request-idempotency.spec.ts`、`plugin/tests/unit/domain/transitions/proposal-position.spec.ts`、`issue.spec.ts`、`decision-candidate.spec.ts`、`decision-acceptance.spec.ts`、`decision-disposition.spec.ts`、`plugin/tests/unit/domain/completion.spec.ts`、`plugin/tests/contract/protocol-schema.spec.ts`、`status-projection.spec.ts`、`http-boundary.spec.ts`、`meeting-runtime.spec.ts`、`tool-registration.spec.ts`、`plugin/tests/unit/runtime/archive.spec.ts`、`plugin/tests/recovery/domain-recovery.spec.ts`、`plugin/tests/client/client-entry.client.spec.ts`、`plugin/tests/unit/scripts/smoke-environment.spec.ts`。
...
- `plugin/scripts/smoke-profile.mjs` 的 selector `convergence`，其 assertion set 精确为 `deterministic-fallback`、`required-unavailable-deduped`、`stall-refocus-replan-exhausted`、`restart-idempotent`
...
| `targetCommit` | 40-char lowercase SHA                                      | 本文 literal；等于 `HEAD`、`origin/main`      |
...
不变量：G0 后始终 `HEAD == origin/main == targetCommit`；G1-G5 不得产生 `plugin/` diff；B/A postconditions 必须同时存在；每个 selector 独立启动且首次 Fail/Restore Fail 立即 STOP；历史 evidence 不外推；`risk-reopen` 不证明 FR-7 closure；C 是两份 readiness 的唯一 writer；证据不得包含 secret、完整环境变量、用户目录或完整绝对临时路径。

## 6. 机械执行步骤

### G0：merged HEAD Investigate→Author→Audit

前置状态：B 已合入 D1-D5；A 已 rebase 到 B 后合入 D6-D10；当前工作树 clean；本文为 tracked file 且含 `targetCommit: UNSET`。

允许修改：仅本文的 targetCommit 行。禁止修改：其他文件；禁止 merge/rebase/cherry-pick。
...
node --version
pnpm --version
rg -n 'const DSH_VERSION = "0\.1\.1-rc\.2"|const PROFILE = "web"|const PROVIDER = "spawn"' plugin/scripts/smoke-profile.mjs
branch_name="codex/runtime-readiness-evidence-${target_sha:0:12}"
! git show-ref --verify --quiet "refs/heads/$branch_name"
git ls-remote origin >/dev/null
! git ls-remote --exit-code --heads origin "$branch_name" >/dev/null 2>&1
git switch -c "$branch_name" origin/main
perl -0pi -e "s/^targetCommit: UNSET$/targetCommit: $target_sha/m" docs/30-designs/RUNBOOK-RUNTIME-READINESS-EVIDENCE.md
test "$(rg -c '^targetCommit: [0-9a-f]{40}$' docs/30-designs/RUNBOOK-RUNTIME-READINESS-EVIDENCE.md)" = 1
test "$(git diff --name-only)" = "docs/30-designs/RUNBOOK-RUNTIME-READINESS-EVIDENCE.md"
```
...
for f in \
  ...
  plugin/tests/unit/scripts/{smoke-environment,smoke-profile-contract}.spec.ts; do test -f "$f"; done
node --input-type=module <<'NODE'
import fs from 'node:fs'
const source = fs.readFileSync('plugin/scripts/smoke-profile.mjs', 'utf8')
const match = source.match(/const SMOKE_SCENARIOS = \[([\s\S]*?)\];/)
...
const expectedAssertions = {
  ...
  convergence: ['deterministic-fallback','required-unavailable-deduped','restart-idempotent','stall-refocus-replan-exhausted'].sort(),
}
for (const [scenario, labels] of Object.entries(expectedAssertions)) {
  for (const label of labels) {
    if (!source.includes(`"${label}"`)) { console.error(`missing ${scenario}:${label}`); process.exit(1) }
  }
}
NODE
...
PASS：全部退出 0；分支名为固定 prefix 加 target SHA 前 12 位；本文恰有一个 literal SHA；A/B exact paths/symbols/tests、12-selector set 和 A assertion set 全存在；进入 G1。

STOP：dirty tree、fetch/pull/branch/SHA失败、任一 postcondition/selector 不一致或出现本文外 diff。不得清理、猜测替代入口或继续。恢复：创建分支后保留分支和本文 diff，不删除或切换。

### G1：focused product tests

前置状态：G0 PASS；仅本文有 diff。允许修改：无。禁止修改：全部文件。

```bash
pnpm --dir plugin exec vitest run tests/unit/protocol/request-idempotency.spec.ts tests/unit/domain/transitions/proposal-position.spec.ts tests/unit/domain/transitions/issue.spec.ts tests/unit/domain/transitions/decision-candidate.spec.ts tests/unit/domain/transitions/decision-acceptance.spec.ts tests/unit/domain/transitions/decision-disposition.spec.ts tests/unit/domain/completion.spec.ts tests/contract/http-boundary.spec.ts tests/contract/tool-registration.spec.ts tests/unit/domain/planning.spec.ts tests/unit/domain/transitions/manager-planning.spec.ts tests/unit/domain/transitions/turn-advancement.spec.ts tests/unit/domain/transitions/speaker-attempt.spec.ts tests/unit/domain/transitions/speaker-submission.spec.ts tests/unit/runtime/manager-fallback.spec.ts tests/unit/runtime/meeting-runtime.spec.ts tests/unit/runtime/turn-runner.spec.ts tests/contract/meeting-runtime.spec.ts tests/contract/protocol-schema.spec.ts tests/contract/status-projection.spec.ts tests/contract/domain-meeting-repository.spec.ts tests/recovery/domain-recovery.spec.ts tests/recovery/recovery.spec.ts tests/unit/runtime/archive.spec.ts tests/client/client-entry.client.spec.ts tests/unit/scripts/smoke-environment.spec.ts tests/unit/scripts/smoke-profile-contract.spec.ts
test -z "$(git diff --name-only -- plugin)"
```

PASS：Vitest 退出 0，每个 file 有通过记录，`plugin/` 无 diff。STOP：首个失败、missing file 或 plugin diff；不得删测试、改 Schema 或缩小命令。恢复：测试自有临时资源；残留资源或 diff 即 STOP。
...
PASS：format、lint、Host/Client typecheck、tests、build、environment、contract、Agent Definition samples、package 全退出 0且 plugin 无 diff。STOP：任一子门禁失败或 tracked diff；不得忽略 aggregate 首败。恢复：ignored build output 由既有 script 管理。
...
前置状态：G0-G5 结果确定且资源已恢复；只有本文有 diff。
...
test "$(git diff --name-only | sort)" = "$(printf '%s\n' docs/30-designs/RUNBOOK-RUNTIME-READINESS-EVIDENCE.md docs/40-readiness/CURRENT-IMPLEMENTATION-COVERAGE.md docs/40-readiness/DSH-RUNTIME-VERTICAL-SLICE-EVIDENCE.md | sort)"
...
PASS：target/date/environment一致；12 selector 均有 record；结果与真实输出一致；只有本文和两份 readiness 有 diff。STOP：需猜测产品状态、丢失首败、外推历史或越界 diff。恢复：保留文档 diff，不 reset。
...
git diff --check
rg -n 'RUNBOOK-RUNTIME-READINESS-EVIDENCE|最终 HEAD Runtime Readiness Evidence' docs .agents plugin
```
...
| D6-D10              | FR-4/FR-6/AC-5/6/20；Protocol Manager/wait；Orchestration       | §4.2 exact symbols                                                           | G1、G2、`convergence` 四 assertions                          | G6                     |
...
| stress/resources    | package/runner cleanup                                          | `test:stress`；`smoke-profile.mjs::restore`                                  | G3-G5                                                        | G6 Not Covered/cleanup |
...
当前时序依赖：仅 B D1-D5 尚未合入、A 尚未在 B 后合入 D6-D10。它们使现在 G0 STOP，但不是产品判断。

执行期机械 STOP：dirty tree；fetch/fast-forward/branch/SHA失败；A/B path/symbol/test/selector postcondition 不匹配；focused/verify/smoke/browser/cleanup失败；环境无法取得 rc.2；readiness 越界或文档检查失败。这些是可复现状态或测试失败，不是产品判断。
...
Audit：`Executable`。Required Structure、权限、Scope/Non-goals、正式依据、exact files/symbols/tests、证据字段、G0 内 SHA 固化与重新 Audit、每步 Prepare/Execute/Assert/Restore、PASS/STOP、失败恢复、双向追踪、readiness 单写和删除条件均已固定。当前未运行 G0-G7、真实 smoke/stress/browser，未写正式 readiness；这是执行阶段事实，不降低决策完备性。
