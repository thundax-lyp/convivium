...
- `plugin/examples/agent-templates/`（迁移后删除）
- `plugin/examples/meeting-agent-definitions/`（新增）
- `plugin/scripts/verify-agent-template-samples.mjs`（替换后删除）
- `plugin/scripts/verify-agent-definition-samples.mjs`（新增）
- `plugin/tests/unit/scripts/agent-definition-samples.spec.ts`（新增）
- `plugin/package.json`

禁止修改：production TypeScript、Persona 职责语义、package `files`。

执行：

1. 把顶层样本目录改名为 `meeting-agent-definitions`，把每个 `ROLE.md` 改名为 `AGENT.md`；不改正文。
2. 按第 4.9 节 key 顺序和第 4.3 节矩阵生成 9 个 JSON；不得从旧 JSON 保留任何值，Persona hash 除外。
3. README 使用第 4.9 节固定 title 和三段，不增加第四段。
4. 把验证脚本改名并严格实现第 4.4 节导出和 CLI；不得实现第二套 DSH grammar，不得访问网络或 DSH registry。
5. 新单元测试只实现第 4.6 节的 exact 18 cases；case 名使用表中 `case`，每个非法 case 只做表中唯一变异，并用 `toEqual(expected)` 断言完整数组。测试用 `try/finally` 清理其精确临时目录。
6. `plugin/package.json#scripts` 删除 key `verify:agent-templates`，在相同 key 顺序位置新增 `"verify:agent-definitions": "node scripts/verify-agent-definition-samples.mjs"`；`scripts.verify` 只把唯一 substring `pnpm verify:agent-templates` 替换为 `pnpm verify:agent-definitions`，其余字符不变；`files` 数组 byte-for-byte 不变。
...
test -z "$(find plugin/examples/meeting-agent-definitions -type l -print)"
test ! -e plugin/examples/agent-templates
test ! -e plugin/scripts/verify-agent-template-samples.mjs
! rg -n "baseInstructionSetRefs|skillSetRefs|toolSetRefs|mcpSetRefs|permissionProfileRef|outputContractRef|modelPolicy|templateId|templateVersion" plugin/examples/meeting-agent-definitions plugin/scripts/verify-agent-definition-samples.mjs plugin/package.json
node - <<'NODE'
const fs = require("node:fs");
const expected = `# Meeting Agent Definition Samples

这些文件是 Convivium Meeting Agent Definition configuration 样本。

它们不是 DSH Agent Preset、Skill registry、Tool registry、MCP registry 或 policy registry，不进入插件发布包，也不证明 capability 已安装。

从 plugin 目录运行 \`pnpm verify:agent-definitions\`，校验九个固定 Definition、文件集合和 AGENT.md SHA-256。
`;
const file = "plugin/examples/meeting-agent-definitions/README.md";
if (fs.readFileSync(file, "utf8") !== expected) {
  process.stderr.write(`${file}: content mismatch\n`);
  process.exit(1);
}
const packageJson = JSON.parse(fs.readFileSync("plugin/package.json", "utf8"));
if (
  packageJson.scripts["verify:agent-definitions"] !==
    "node scripts/verify-agent-definition-samples.mjs" ||
  Object.hasOwn(packageJson.scripts, "verify:agent-templates") ||
  packageJson.scripts.verify.split("pnpm verify:agent-definitions").length !== 2 ||
  packageJson.scripts.verify.includes("verify:agent-templates") ||
  JSON.stringify(packageJson.files) !== JSON.stringify(["lib", "cordis.patch.yml", "README.md"])
) {
  process.stderr.write("plugin/package.json: verifier scripts mismatch\n");
  process.exit(1);
}
NODE
```

PASS：CLI 输出 exact PASS；focused test exact 18 passed、0 skipped；样本树无 symlink；README byte-for-byte 匹配第 4.9 节；旧目录/脚本不存在；禁止字段无命中。

STOP：需要发明 DSH preset/tool/skill 名称、改变 Persona 职责，或让样本进入发布包。

### T4：用 DSH 原生 service 类型替换 shadow interfaces
...
禁止修改：函数 body、authorization assertion、DSH 调用参数、返回值、protocol/repository/domain。

执行：

1. 修改前只运行下述“修改前门禁”；输出必须 exact 为 `PASS 11 shadow-interface files`。不得目测比较、在修改后重跑该门禁或动态扩展 allowlist。
...
const actual = execFileSync(
  ...
  [
    "-l",
    "ContinuableStarter|ContinuableFollowupRuntime|ContinuableLifecycleRuntime|ArchiveSessionRuntime|ContinuableInspectionRuntime|MeetingToolRegistry|SubagentProviderRegistry",
    "plugin/src",
    "plugin/tests",
  ],
  { encoding: "utf8" },
...
process.stdout.write("PASS 11 shadow-interface files\n");
NODE
```

2. 在 `session-adapter.ts` 的现有 `@deepseek-ai/dsh-subagent` type import 中增加 `SubagentRuntime`，删除只被 shadow interface 使用的 `SubagentDescendantListEntry`、`SubagentFollowupOptions`、`SubagentInterruptAuthority`、`SubagentListEntry`。把每个 input 的 `runtime` 精确替换为相应 `Pick<SubagentRuntime, ...>`：starter=`"startContinuable"`，followup=`"followup"`，lifecycle=`"interrupt" | "drainContinuableChildren"`，archive=`"listChildren"`，inspection=`"listDescendants"`。`requireContinuableProvider()` 参数改为 `Pick<SubagentRuntime, "getProvider">`。
   对 T4 允许修改列表中除 `dsh/index.ts`、`tools/index.ts`、`register-tools.ts` 外的每个使用 `SubagentRuntime` 的文件，增加 exact type import `import type { SubagentRuntime } from "@deepseek-ai/dsh-subagent";`；若该 package 已有 type import，则只把 symbol 合并进现有 import。不得从内部 `lib/types/*` 路径 import。
3. `meeting-runtime.ts#MeetingCreationRuntimeDependencies.continuable` 改为 `Pick<SubagentRuntime, "startContinuable">`。
4. `application-service/index.ts#CreateStatusRuntimeOptions.continuable` 改为 `Pick<SubagentRuntime, "startContinuable" | "followup" | "listDescendants"> & Partial<Pick<SubagentRuntime, "listChildren" | "interrupt" | "drainContinuableChildren">>`；不得扩大 optional/required method 集合。
5. `meeting-control.ts#PauseRecoveryDependencies.lifecycle` 改为 optional `Pick<SubagentRuntime, "interrupt" | "drainContinuableChildren">`。
6. `meeting-dispatch-service.ts#MeetingDeliveryDispatcherOptions.continuable` 改为 `Pick<SubagentRuntime, "followup">`；`scanMeetingMailTimeouts().continuable` 改为 `Pick<SubagentRuntime, "followup"> & Partial<Pick<SubagentRuntime, "interrupt">>`。
7. `meeting-archive-service.ts#ArchiveCleanupRuntime` 与 `meeting-session-service.ts#ArchiveCleanupRuntime` 都改为 `Pick<SubagentRuntime, "listChildren" | "interrupt" | "drainContinuableChildren">`；保留该 Convivium archive capability alias 名称。
8. `meeting-recovery-service.ts#MeetingRecoveryDependencies.inspection` 与 `CaptainRebindDependencies.inspection` 改为 `Pick<SubagentRuntime, "listDescendants">`。
9. `register-tools.ts` 增加 exact import `import type { ToolRuntime } from "@deepseek-ai/dsh-tools";`，把共享 registration options 中的 `registry` 改为 `Pick<ToolRuntime, "register">`；删除并停止 export `MeetingToolRegistry`。
10. 删除 7 个 shadow interface 及其 `dsh/index.ts`/`tools/index.ts` re-export。测试文件不需要修改；若 typecheck 要求修改未列出的 mock 或 production 文件，STOP。
...
pnpm --dir plugin exec vitest run tests/unit/dsh/session-adapter.spec.ts tests/contract/tool-registration.spec.ts tests/unit/runtime/meeting-runtime.spec.ts tests/recovery/recovery.spec.ts tests/unit/runtime/archive.spec.ts
! rg -n "SubagentProviderRegistry|ContinuableStarter|ContinuableFollowupRuntime|ContinuableLifecycleRuntime|ArchiveSessionRuntime|ContinuableInspectionRuntime|MeetingToolRegistry" plugin/src plugin/tests
for file in plugin/src/dsh/session-adapter.ts plugin/src/runtime/meeting-runtime.ts plugin/src/runtime/application-service/index.ts plugin/src/runtime/application-service/meeting-control.ts plugin/src/runtime/services/meeting-archive-service.ts plugin/src/runtime/services/meeting-dispatch-service.ts plugin/src/runtime/services/meeting-recovery-service.ts plugin/src/runtime/services/meeting-session-service.ts; do rg -Fq "Pick<SubagentRuntime" "$file"; done
rg -Fq 'Pick<ToolRuntime, "register">' plugin/src/tools/register-tools.ts
```

PASS：修改前 Node 断言输出 exact `PASS 11 shadow-interface files`；typecheck 与 focused tests 通过；shadow symbol 无命中；原生 `Pick` 在验证命令列出的 8 个 Subagent 边界文件中均有命中，`ToolRuntime` 在 tool registration 文件中有命中。

STOP：需要改变 production 行为或要求 DSH 添加新 method；不得以 `any`、本地 method interface 或放宽 optionality 解决。
