...
允许修改：`plugin/meeting-roles/cordis.patch.yml`、`plugin/meeting-roles/presets/convivium/agent.cordis.yml`、`plugin/scripts/smoke-profile/index.mjs`、`plugin/scripts/smoke-profile/probe/scenarios/meeting-roles.js`、`plugin/scripts/smoke-profile/probe/index.js`、`plugin/tests/unit/scripts/meeting-roles-smoke.spec.ts`、`plugin/tests/unit/scripts/smoke-profile.spec.ts`；Definition Interface 的部署说明、Role Composition Design 的 Native deployment resources、两份 operations、包内 README、本文件和 TODO。不新增文件/依赖。T5 成功后允许同步 Requirements FR-14、Definition Interface、Architecture、Role Composition Design 中本任务的过期实现状态语句；不改其业务契约。
...
5. 同步独立部署命令导出非敏感资源根变量、patch 整体替换语义；迭代修复可在 focused gate 后运行 meeting-roles 定向真实探针以定位同范围问题，此结果不替代 T5。修复单独提交后，仍按 T5 四条顺序命令完成最终复验。
...
2026-09-08 MAD-06 PASS：旧 role-composition smoke 已拆分模型 map，完整派生 persona 与冷恢复断言同步；7 个 focused tests、diff check PASS，保留禁止另一身份标记、禁用 body 零调用与父配置隔离。真实双 Host 验收待 MAD-10。

2026-09-08 MAD-07 PASS：新场景 wrapper/probe 路由、同包解包和 patch 顺序、八身份容量及 finally 清理已接线；三个 smoke suites 共 52 tests PASS。场景模块由 MAD-08 完成，T4 尚未整体通过；新 selector 尚不可用于真实验收。
...
2026-09-08 T4b 探针修复：cd9b105 全 verify（1135 tests）与 role-composition（12529ms，Host 72275/72288，restore=PASS）通过；meeting-roles 已挂载 Captain，但在 probe 直接访问未 inject 的 agentPresets 时失败。改用既有 prepareRoleSmoke 同样的 ctx.get 显式服务查询，增加进入真实创建边界前的回归。失败场景 root convivium-dsh-smoke-cYEBJu 由 finally 清理；没有改变生产服务注入或会议权限。

2026-09-08 T4b 定向探针定位：资源和九 Session 创建通过，但自动 Captain 未选择模型，原生 turn/end 报无 provider/model，180 秒等待失败。探针以 CreateAgentOptions.agentOptions 显式选择已确认原生 deepseek-official/deepseek-v4-flash，子会话按原生继承；不把模型写回 Definition/Preset。新增 turn/end error 即时失败回归。临时 root convivium-dsh-smoke-jSShuU 已由 finally 清理。

2026-09-08 T4b 生命周期修复：九次真实 Skill 加载均通过，旧探针等待 child idle 后持有已释放 Agent 调用 search，原生 provider 记录请求时报 session not live。研究/权限/status 检查改在目标 child 成功 Skill 的 tools/post-execute 回调内、返回原 decision 前完成，finally 注销回调；不修改 Skill 结果或生产生命周期。原生 ToolFailure 错误码按 error.info.code 读取。

2026-09-08 T4b 外部环境断点：修复后的 GitHub 原生搜索通过，web_fetch 返回 WEB_BLOCKED_URL，hostname resolves to a non-public IP address。只读 DNS 检查三个目标域名均为 198.18.*，本机 Shadowrocket MacPacketTunnel 提供 Fake-IP DNS。当前请求用户确认仅这三个域名的临时公网解析配置，未改代理配置、原生公网检查或成功判据；MAD-10/11 保留。
