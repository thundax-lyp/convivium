...
每场 Meeting 的唯一专职 evidence reviewer 使用一个 meeting-owned coordinator Session。Runtime 在投递 review effect 前通过 application 原子创建持久 `ReviewBatchClaim`，固定 Round、reviewer、确切 versionIds、sourceEffectId 与 expiry；只有认领成功才向 coordinator 提供该待审集合。coordinator 对每个 version 只调用一次 reviewer 专用 `convivium_run_review_worker`；该工具以当前 reviewer Agent 为 parent，调用 `spawn` provider 创建无工具权限的 DSH 原生 one-shot worker，并把固定 Review JSON Schema 作为 `outputSchema` 传入。工具只接受 provider 已验证的 `structured` 结果并核对 versionId，第一轮各维 `baselineEvidenceIds: []` 无需特殊降级。worker 不是 MeetingIdentity、没有 Meeting command authority，全部结果回到 coordinator 后携带 claimId 由 `submit_review_batch` 原子提交并移除 claim；不持久化 worker 内部状态。有效 claim 阻止不同 dispatcher 实例重复唤醒：其它 effect 去重完成，原始 source effect 延后到 claim expiry 再领取，避免按 poll 周期空转和阻塞后续 effect。若 coordinator turn 超时、中断、投递失败或正常结束但没有完整 Review，dispatcher 通过 Runtime-only command 精确撤销该 claim，使迟到提交失效并允许重新认领；实际重新执行使用 outbox 的有限 attempt budget，耗尽后把 effect 持久标为 `failed`，不再空转。进程崩溃等未观察到的结束仍由 expiry 兜底。冷恢复从 MeetingState 的 claim 与最终 Review 恢复调度。Reviewer Definition 使用专用 Preset/Skills，allow-list 不再暴露通用 `subagent`，并可引用 Host-approved 的其他材料读取、代码核验、Web/GitHub/arXiv 查询和运行验证能力；具体工具由版本化 Definition 与 Host 配置拥有，Convivium 不以“工具最多”授予权限。Review delivery 的 sent/failed 尝试进入 caller-filtered read model，使 Manager 看状态、作者看自身 delivery、reviewer 看全部 delivery；其他身份不能借投递状态获知未公开 Review。
