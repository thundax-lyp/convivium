...
`convivium_read_meeting` 与 command tools 分离：tool adapter 先用可信 `exec.agent` 解析 active ownership，并要求请求的 `meetingId` 与 ownership 一致；identity reader 再核对 committed MeetingIdentity 的 `identityId`、role 和 `sessionOwnershipId`，最后复用 `projectMeetingView` 生成 identity projection。它不调用 local Web `read`，因此不能暴露 local 完整投影；也不创建新的持久状态、事件、receipt 或 outbox。Manager 与 Evidence Reviewer 使用显式 allow-list；五类 Contributor 使用 deny-list 隐藏 Captain、Manager 和 Reviewer command，仅保留共享 Preset 能力以及 `convivium_read_meeting`、`convivium_raise_hand`、`convivium_submit_evidence` 等未被拒绝的 Contributor 入口。七个角色 Skill 均要求收到 notice 后先读取事实再决定处理或忽略。
...
每场 Meeting 的唯一专职 evidence reviewer 使用一个 meeting-owned coordinator Session。Runtime 在投递每个 review effect 前通过 application 原子创建单 version 的持久 `EvidenceReviewClaim`，固定 Round、reviewer、versionId、sourceEffectId 与 expiry，并把该版本从 `submitted | validation_failed | validation_cancelled` 置为 `validating`；只有认领成功才向 coordinator 提供该版本。coordinator 只调用一次 reviewer 专用 `convivium_run_review_worker`；该工具以当前 reviewer Agent 为 parent，调用 `spawn` provider 创建无工具权限的 DSH 原生 one-shot worker，并把固定 Review JSON Schema 作为 `outputSchema` 传入。工具只接受 provider 已验证的 structured 结果并核对 versionId，第一轮各维 `baselineEvidenceIds: []` 无需特殊降级。worker 不是 MeetingIdentity、没有 Meeting command authority；结果携带 claimId 由 `submit_evidence_review` 独立提交，成功把版本置为 `validated` 并移除 claim，不持久化 worker 内部状态。

有效 claim 阻止不同 dispatcher 实例重复唤醒同一 version：其它 effect 去重完成，原始 source effect 延后到 claim expiry。DSH `sendMessage` 返回值只证明 inbox acceptance，不证明 coordinator turn 已开始或结束；该返回不触发完整性检查或 claim 撤销。可观察到超时、中断或投递失败时，dispatcher 调用 `fail_evidence_validation` 精确消费 claim、递增 failureCount 并记录原因；无法观察 turn 结束时保留 claim 至 expiry，再记录 `review_timeout`。重试由 EvidenceStatus 和固定失败上限 5 决定；Outbox attempt 只约束普通运输失败，不提前终止尚未耗尽的验证预算。Meeting pause 把 `submitted | validating` 版本置为 `validation_cancelled`、取消 in-flight claim 并完成旧 effect，resume 重新生成 review effect；取消不消耗失败预算，也不在暂停期间轮询。冷恢复从 MeetingState 的 EvidenceStatus、claim 与最终 Review 恢复调度。Reviewer Definition 使用专用 Preset/Skills，allow-list 不再暴露通用 `subagent`。Manager 通过脱敏 validation status projection 读取当前版本状态与失败次数，不读取证据正文、资料 ID 或评分；Review delivery 的 sent/failed 尝试仍按 caller 过滤。其他身份不能借状态或投递信息获知未公开证据。
