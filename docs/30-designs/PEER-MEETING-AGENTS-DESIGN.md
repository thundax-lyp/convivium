...
Captain 是本地用户，当前部署只有单 Host 单用户，不设置 Captain Agent 或原 Session 授权锁。生产入口为 `plugin/src/remote/index.ts::ConviviumRemoteService.control` → `plugin/src/runtime/meeting-lifecycle.ts` 的 LocalMeetingWebRuntime.control → application。Remote 仍只在 webServer.host=127.0.0.1 时注册；来源固定 `{channel:"loopback_remote",principalId:"local-controller"}`。用户的创建、十项控制与 pause/resume/end 使用同一 action 白名单与版本/幂等/领域校验；start_archive 及归档结果由 runtime_recovery 执行。Agent tools 仅用于会议身份操作，不注册 Captain 控制工具，并删除旧 create tool。

`MeetingCommandExecutionContext` 删除 captainParent；创建从可信 adapter 得到 `creator:{kind:"local_user",principalId:"local-controller",sourceSessionId?:string}`。当前 Remote 不提供 Session 来源，sourceSessionId 缺席；后续 Host 原生用户输入 adapter 只有具备真实用户来源证明才可填写，本次不实现该 adapter、自然语言解析或自动代理授权。原 Session 关闭、重开面板、Host 重启均不影响当前本地用户控制。
