...
├── pnpm-lock.yaml
├── cordis.patch.yml
├── meeting-roles/                 # 同包原生 Preset/Skills/Definition 与部署 patch
├── tsconfig.json
...
| `meeting-roles/` | 同包交付的 Definition、共享 Preset、九个原生 Skills 与显式部署 patch；由 DSH Loader 应用，验收范围见 [Coverage](../40-readiness/CURRENT-IMPLEMENTATION-COVERAGE.md#shared-preset-role-composition) |
| `scripts/verify-agent-definition-samples.mjs` | 校验九角色部署文件集合、原生引用与 Skill 正文；旧 AGENT.md hash 规则已移除 |
