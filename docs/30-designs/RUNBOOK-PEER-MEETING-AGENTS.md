...
- T20 资源与旧入口清理：删除共享 convivium Preset 及七旧角色 Skill；保留目标七 Preset/七 AGENTS/五 Skills。两个资源 verifier 与 deployment 合同改为目标闭集，插件 export 合同同步移除 assertContinuableProvider（允许范围追加 verify-plugin-contract.mjs）。Manager/Reviewer AGENTS 保留命令参数层级、不可变版本及 one-shot worker 限制并更新哈希。指定旧生命周期关键词搜索零命中（退出 1）；build、verify:contract、verify:agent-definitions、verify:package 已通过。真实 profile 已启动，尚无 PASS 结论。
- T21 回归修复：owner 的 descriptor 校验移入 role-composition/dsh-capabilities.ts，消除 DSH 到 Repository 的运行时依赖；资源比较改用结构等值。原生 owner 测试的禁调用模型桩改为显式抛错返回签名。production-import-graph 将文件系统禁令保留在持久化 Repository，Runtime 已按 T5/T8 的正式资源加载要求读取 AGENTS/Skills；插件整体仍禁止物理存储依赖。允许范围追加上述 owner/capabilities 和两项边界/原生测试；模块边界、原生 owner、全部测试与 host/client 类型检查通过。
