...
允许修改：无受 Git 管理的仓库文件；只允许创建当前完整 `git HEAD` 对应的 `dsh-workspace/web-ui/<git HEAD>/` 子工程及其中的 profile、DSH workspace、对话和 SQLite。禁止修改：现有 DSH profile、现有 release、其他 `dsh-workspace/` 子工程、仓库源码、测试和文档。

执行：

1. 在仓库根的同一 shell 运行以下命令。完整 `git HEAD` 是本次验收子工程的固定目录名；目标已存在时 STOP，不覆盖、不删除、不改用 `/tmp`。首次创建的目录仅属于本次验收；相同版本 release 不允许覆盖。

```bash
set -eu
meetings_view_repository_root="$(git rev-parse --show-toplevel)"
test "$PWD" = "$meetings_view_repository_root"
test -z "$(git status --short)"
command -v lsof >/dev/null
test -z "$(lsof -nP -iTCP:31828 -sTCP:LISTEN)"
meetings_view_revision="$(git rev-parse HEAD)"
meetings_view_root="$meetings_view_repository_root/dsh-workspace/web-ui/$meetings_view_revision"
meetings_view_workspace="$meetings_view_root/workspace"
meetings_view_install_root="$meetings_view_root/convivium-user"
test ! -e "$meetings_view_root"
mkdir -p "$meetings_view_root"
chmod 700 "$meetings_view_root"
CONVIVIUM_INSTALL_ROOT="$meetings_view_install_root" ./scripts/install-from-source.sh --workspace "$meetings_view_workspace"
```
...
test "$meetings_view_root" = "$(git rev-parse --show-toplevel)/dsh-workspace/web-ui/$meetings_view_revision"
test -d "$meetings_view_workspace"
test -d "$meetings_view_install_root/dsh-home/sessions"
...
```
PASS：十二条命令退出 0，记录含 HEAD、工作树、release、唯一 artifact hash 和 Captain Session ID，且记录唯一 fixture A/B `meetingId` 并人工确认 A=`running`、B=`archived/complete`。STOP：工作树不干净、31828 端口被占用、安装失败、固定目标路径预先存在、artifact 不是唯一文件、新 profile 不是 `patchReload: startup`、固定凭据不存在/权限不符/为空、安装器占位文件不是预期内容、Provider/Preset 缺失、fixture 无法通过正式入口建立、fixture ID 不唯一或安装期间源码变化；保留命令、Host 输出和子工程，不改用旧 profile/smoke/复制 SQLite 或复制 key。失败恢复：保留失败子工程并 STOP，不自动删除或覆盖；修复形成新 HEAD 后在其对应的新固定子工程重跑 T14。

### T15：真实 DSH Web 人工门禁与 readiness

前置状态：T14 PASS；使用 T14 记录的 `meetings_view_install_root`、Captain Session ID 和两个 `meetingId`，fixture A=`running`、fixture B=`archived/complete`。

允许修改：`docs/40-readiness/CURRENT-IMPLEMENTATION-COVERAGE.md`。禁止修改：产品代码；发现缺陷时回到对应 T1–T12 修复并重跑 T13–T15，使用修复后新 HEAD 对应的固定子工程。

执行：

1. 人工操作者运行 `(cd "$meetings_view_install_root" && ./start.sh)` 并保持前台；不得重新安装、覆盖 release、复制 SQLite 或改用日常 profile、`/tmp` profile。
...
5. fixture A 为 running 时在 Host 终端 Ctrl+C，保持 Browser 页面；确认 stale 且 pause/end 禁用。再从同一 install root 运行同一 `start.sh`；确认 reconnecting stream 自动重开、完整 list + fixture A detail 成功后 stale 消失且 pause/end 恢复。按 T14 记录的 Captain Session ID 打开同一对话，确认其中创建 fixture A/B 的已提交消息仍可读；缺失即 FAIL，不用 Meeting SQLite 中的事实替代对话验证。
...
8. Ctrl+C 停止 Host；保留 profile、对话和 SQLite，待 T16 核对。只有清单全部 PASS 后才修改 readiness：在 Feature Coverage 新增或替换唯一 `MO-FR-17 | Meetings View 概览与时间线` 行，状态写 `已实现`，当前覆盖写 Navigator、共享 Header、七组 Overview、五泳道 Timeline、generation 恢复和 `zh/en`，缺口只保留 Browser 自动化/性能；在 Executed Validation 追加一行 T13 自动命令证据和一行 T14–T15 真实 Web 证据，后者必须包含日期、HEAD、`git status --short`、workspace/install root/release、artifact SHA-256、Captain Session ID、fixture A/B、断线重连、对话重开及人工清单结果，并注明固定子工程待 T16 核对后保留、固定 `dev.env` 未复制。不得改写既有历史证据。
...
git diff --check
```
PASS：人工清单和两命令全部 PASS，readiness 证据完整。STOP：缺 profile/fixture/key，端口被占用，Host 失败，任一清单 FAIL，或人工未确认；保留精确 Host 输出和固定子工程供失败诊断，不触碰仓库根 `dev.env`。失败恢复：代码缺陷回对应 T1–T12；环境/fixture 缺陷回 T14；之后必须重跑 T13–T15。

### T16：核对并保留固定验收工程

前置状态：T15 PASS；Host 已停止，readiness 已记录 T14–T15 证据，固定 `dev.env` 位于仓库根且未被复制。
允许修改：只修改 `docs/40-readiness/CURRENT-IMPLEMENTATION-COVERAGE.md` 内 T14–T15 本次证据行的保留状态。禁止修改或删除：T14 创建的 `dsh-workspace/web-ui/<git HEAD>/` 子工程及其中的对话、DSH_HOME、workspace、SQLite、release 和凭据链接；仓库根 `dev.env`；其他仓库文件。

执行：先从 T14 记录恢复 `meetings_view_revision`、`meetings_view_root`、`meetings_view_workspace` 和 `meetings_view_install_root`；确认它们与记录的绝对路径完全一致，再运行下列只读检查。`lsof` 查到任何占用验收端口的进程时 STOP，不终止该进程。不得删除或移动验收子工程。
...
test "$meetings_view_workspace" = "$meetings_view_root/workspace"
test -d "$meetings_view_workspace"
test -d "$meetings_view_install_root/dsh-home/sessions"
test -n "$(find "$meetings_view_install_root/dsh-home/sessions" -type f -name session.jsonl.zstd -print -quit)"
test -f "$meetings_view_install_root/convivium-storage.sqlite"
test -f "$meetings_view_install_root/release"
test "$(sed -n '1p' "$meetings_view_install_root/workspace-path")" = "$meetings_view_workspace"
test -L "$meetings_view_install_root/dev.env"
test "$(readlink "$meetings_view_install_root/dev.env")" = "$(git rev-parse --show-toplevel)/dev.env"
...
检查成功后，只将 T14–T15 本次证据行中的“固定子工程待 T16 核对后保留”更新为“T16 PASS，固定验收子工程及对话、会议 SQLite 已保留；仓库根 `dev.env` 保留且未复制”，不改写其他证据。
...
git diff --check
```
PASS：只读检查与六条验证命令退出 0，固定 `dev.env` 仍存在，验收子工程、对话与会议 SQLite 均保留，readiness 记录保留结果。STOP：路径、链接目标、对话文件、SQLite 或 Host 状态无法核对，固定 `dev.env` 不存在，或文档检查失败；保留实际路径和错误，不删除任何子工程。失败恢复：不自动清理或移动数据；按精确路径诊断后重新执行只读检查。

### T17：收口并删除 RUNBOOK

前置状态：T16 PASS，readiness 已包含 T13–T15 的真实证据及固定验收子工程的保留说明。
...
| install/Web | T13 通过的工作树构建为唯一 artifact，新 profile 采用 `patchReload: startup`，在 `dsh-workspace/web-ui/<git HEAD>/` 的隔离持久子工程验证真实 Host 与重启恢复，凭据只读取固定 `dev.env`，验收后保留对话和会议数据 | T14–T16 |
...
Scope 全部有代码/focused test/PASS，Non-goals 未引入，T13 自动验证、T14 安装、T15 人工门禁与 readiness、T16 固定验收工程保留核对全部 PASS 后，才由 T17 删除本文。长期结论属于 requirements/interfaces/designs，验证证据属于 readiness。
...
本次 HMR 修复的聚焦安装契约测试和 `pnpm --dir plugin verify` 是既有历史证据，不作为本次文档修订的重跑结果；先前 `/tmp` 诊断 profile 中的 `patchReload: startup` 曾使真实 DSH Web 监听固定端口，但不算 T14 PASS。T14–T15 仍须从修复后的干净工作树，在本 RUNBOOK 固定的持久子工程重新安装、建立 fixture 并完成人工 Web 清单。

固定路径与保留规则修订已运行 `node .github/scripts/check-doc-links.mjs`（478 条本地文件链接、0 错误）及 `git diff --check`（PASS）。T14–T16 的真实 Host、对话重开和人工门禁尚未执行，不能以本次文档检查代替其 PASS。
