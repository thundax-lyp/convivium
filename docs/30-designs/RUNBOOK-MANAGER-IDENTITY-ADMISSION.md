...
状态：Executable；当前 checkout 因共享目标 Meeting command/Repository 管线尚未落地而在 T0 正常 STOP，前置落地后重新从 T0 执行并在 PASS 后直接进入 T1，无需再次选择入口或等待作者判断。审计结论和当前 STOP 证据在文末。执行分支：codex/role-recommendation-runbook。建立日期：2026-09-16，最近审计日期：2026-09-16。本 RUNBOOK 不授权提交、推送、建 PR 或合并。

## 执行者契约与目标

执行者严格按 T0→T9 顺序工作；当前 checkout 只运行只读 T0并按已知缺失文件 STOP，前置代码由独立上游切片落地后重新运行同一 T0，全部 PASS 即直接进入 T1。T1–T8 每步先写该步指定行为测试并观察目标 RED，再改允许文件、运行指定 GREEN。PASS 必须同时满足命令退出码 0 和该步可观察断言。STOP 立即停止，报告最后 PASS 步骤、触发条件、文件/symbol、最小复现命令及实际输出；不得临场选择文件、替代入口、放宽类型/Schema/断言、跳过 RED 或把旧 Turn 代码当作目标。已有用户改动不回滚；本 RUNBOOK 的 Git 交付边界仍需另行授权。
...
当前 plugin/src/domain/meeting-state-v1.ts::MeetingState、plugin/src/domain/meeting-state-v1-validation.ts::validateMeetingStateV1、plugin/src/domain/meeting-state-v1-transitions.ts::transitionMeetingStateV1 和 `transitions/*-v1.ts` 已实现目标聚合、validator 与 T0—T7 纯 Domain 行为并从 domain 入口导出，但 MeetingState 仍没有 identityRecommendations，旧 Runtime 也没有调用这些目标转换。plugin/src/repository/meeting-repository-port.ts::MeetingRepositoryPort.execute 与 plugin/src/repository/domain/domain-meeting-repository.ts::DomainMeetingRepository.execute 已提供 legacy 使用的授权后 receipt、expected version、state/event/receipt/outbox 原子 commit 基元；目标 adapter 必须复用它们，不得创建第二个 Storage Domain 或 Repository 实现。plugin/src/role-composition/model.ts::MeetingAgentDefinitionV1 和 plugin/src/role-composition/resolve.ts::resolveMeetingRoles 是初始身份组成；plugin/src/runtime/services/agent-catalog.ts::captureManagerCatalogBinding、plugin/src/domain/transitions/manager-planning.ts::submitManagerPlan、plugin/src/domain/transitions/attendance-rejection.ts::rejectAttendanceRecommendation 是 legacy。当前仍没有目标准入 command、Definition descriptor admission port、目标 Repository adapter 或目标 runtime recovery；[Coverage](../40-readiness/CURRENT-IMPLEMENTATION-COVERAGE.md#functional-coverage) 的目标 Domain 已对齐不等于 Runtime/Repository 已接入。
...
T0 的共享目标依赖是 [Meeting Design §Common Command Pipeline](./MEETING-DESIGN.md#common-command-pipeline) 中同一 Runtime 入口、MeetingState 无损 codec、MeetingCommitV1 原子 Repository adapter、outbox dispatcher、caller ownership、target projection/Archive/end/recovery。作者为接口接线固定唯一依赖路径：
...
其中 `MeetingCommandRepositoryPortV1` 只能是现有 `MeetingRepositoryPort.execute/read/recover/claimOutbox/completeOutbox` 的目标类型 adapter，不得实现第二套持久化、receipt 或 outbox；`recoverMeetingCommandsV1` 只能复用现有 `createMeetingRehydrationService` 和 Repository recovery 结果，不得扫描物理存储。上述文件/symbol 或 T0 行为断言不存在时执行 T0 STOP，不能让本准入切片补建或替换共享管线。上游只能按这些固定入口落地；若采用不同入口，T0 必须 STOP 并报告路径差异，不允许执行者自行改名接线。这是代码前置依赖，非产品语义选项。
...
执行：逐项验证固定的共享目标文件/symbol、共享 core contract 的行为断言与 host typecheck。T0 不以源码字符串命中代替运行断言；symbol 检查只负责尽早定位错误入口。
验证：
~~~bash
set -e
...
rg -q 'endMeetingV1' plugin/src/domain/transitions/meeting-end-v1.ts
rg -q 'startMeetingArchiveV1' plugin/src/domain/transitions/meeting-archive-v1.ts
...
~~~
PASS：全部命令退出 0，且 core contract test 真实调用固定 symbol 并断言：target codec 对完整 MeetingState 无损 round-trip、缺 required 字段返回 INCOMPATIBLE_VERSION；caller ownership 在 receipt 前检查；同 principal/request/payload 重放原结果、异 payload IDEMPOTENCY_CONFLICT；stale version 不提交；一次 accepted commit 原子保存 state、resultingState fact、receipt 与 outbox；任一 transition/Storage 失败均无半提交；projectMeetingViewV1 只读 committed snapshot；end/archive 使用同一 command pipeline；recoverMeetingCommandsV1 只恢复同一 Repository 的未完成 outbox 和可证实 ownership。满足这些断言后直接进入 T1，不再请求作者选择入口。
STOP：任一文件/symbol 缺失、adapter 未复用 `MeetingRepositoryPort`、测试未实际覆盖任一上述断言、类型仍是 legacy 或命令失败；报告首个失败路径、缺失断言和输出。当前 checkout 必然 STOP 于首项。该步只读，无数据库/Session 回滚。

### T1：目标协议与 filtered read schema

前置状态：T0 PASS；执行者按 T0 的固定行为断言通过后直接进入本步。
...
前置状态：T1 PASS。
允许修改：plugin/src/domain/meeting-state-v1.ts、plugin/src/domain/meeting-state-v1-validation.ts、plugin/src/domain/meeting-state-v1-transitions.ts、plugin/src/domain/index.ts、plugin/src/domain/transitions/index.ts、plugin/src/domain/transitions/meeting-create-v1.ts、plugin/src/repository/meeting-command-repository-v1.ts、plugin/tests/fixtures/meeting-state-v1.ts、plugin/tests/unit/domain/meeting-state-v1-validation.spec.ts、plugin/tests/unit/domain/meeting-state-v1-transitions.spec.ts；新增 plugin/src/domain/transitions/meeting-identity-v1.ts、plugin/tests/unit/domain/meeting-identity-v1.spec.ts。
禁止修改：plugin/src/domain/model.ts 与旧 manager-planning/attendance-rejection；不得把 Session handle 放入 Domain。
执行：测试先行 RED；在 meeting-state-v1.ts 唯一声明 IdentityRecommendationV1 和目标聚合 identityRecommendations:readonly IdentityRecommendationV1[]，补 MeetingIdentityV1.definitionHash?/sessionOwnershipId?；同步扩展 target validator schema、typed refs/终态不变量、共享 fixture 与现有 dispatcher action union，缺失或畸形 identityRecommendations 必须 fail closed，不能由 codec/validator补空数组。meeting-create-v1.ts 初始化空 identityRecommendations，Repository target codec 无损读写该 required 数组且旧缺字段返回 INCOMPATIBLE_VERSION（目标无 implicit migration）；ArchivePackageV1.identityProvenance 与归档构造一起由 T8 添加。新 transition 文件唯一实现 recommendIdentityV1(state,action,managerId,allocatedIds,now)、recordIdentityAdmissionResultV1(state,recommendationId,result,now)，并由 transitionMeetingStateV1 对两个 action 做唯一分派；result 仅为 Runtime 内部 plain data，不导入 DSH/Repository。Domain 只检查 lifecycle、Agenda、candidate 重复、intent status；admitted result 必须比对 admissionId=id、meetingId=state.id、identityId/childSessionId/Definition id/version/hash 与 intent 固化值，rejected result 的 failureCode 只做非空检查，Runtime 必须在调用前用 Protocol RoleErrorCodeV1 完成 enum 校验；ownershipId/descriptorId/displayName 必须非空。caller/Catalog/version 与实际 ownership/Session/descriptor 归属由 Runtime/Repository 校验；Domain rejection 只返回 errorCode 与不变输入 state，不产出 fact/effect；Runtime 同码映射为 MeetingErrorV1 且不提交。accepted reject 只写 rejected，accepted admit 只写 intent，完成时 ownership/id/hash 精确匹配，并用 context.displayName 激活普通 Participant。domain/index.ts 与 transitions/index.ts 只导出 Runtime 需要的两个 transition、共享 dispatcher action 和目标类型。单元测试断言新建目标状态含空数组、validator/Repository encode/decode 后数组不丢失、缺失/畸形数组拒绝、非法转换返回 rejected/errorCode 且无 state/fact/effect 变更、reject 无身份、admit 未调度、成功唯一 active、失败无身份、终态不可变；更新既有 fixture/dispatcher tests 只补 required 字段与两个新 action，不改旧行为预期。
验证：
~~~bash
pnpm --dir plugin exec vitest run --project host tests/unit/domain/meeting-identity-v1.spec.ts tests/unit/domain/meeting-state-v1-validation.spec.ts tests/unit/domain/meeting-state-v1-transitions.spec.ts
pnpm --dir plugin typecheck:host
~~~
PASS：退出 0；测试断言上述状态和反例，未列入本步允许文件的 legacy Transition 未修改。
...
允许修改：plugin/src/projection/meeting-view-v1.ts、plugin/src/domain/meeting-state-v1.ts、plugin/src/domain/meeting-state-v1-validation.ts、plugin/src/domain/transitions/meeting-archive-v1.ts、plugin/src/repository/meeting-command-repository-v1.ts、plugin/tests/fixtures/meeting-state-v1.ts、plugin/tests/unit/domain/meeting-state-v1-validation.spec.ts、plugin/scripts/smoke-profile/index.mjs；新增 plugin/tests/contract/meeting-identity-view-v1.spec.ts。
禁止修改：Client/HTTP 页面、完整 Definition/Skill正文/Session/descriptor/status secret投影、新 Profile installer。
执行：测试 RED；在 meeting-state-v1.ts 添加 required ArchivePackageV1.identityProvenance，同步 target validator/archive schema、fixture 与 validator tests，并由 archive transition 在创建时固化；缺失 identityProvenance 时 validator fail closed，Repository target codec 将该持久格式缺失映射为 INCOMPATIBLE_VERSION，均不得补空数组。target projection输出已激活 IdentityView与仅 Manager/Captain/local 可见的 IdentityRecommendationView、Manager-only安全 Catalog；ArchivePackage.identityProvenance只固化已激活 identity的 ID/name/role/Definition id/version/hash，不包含未完成 intent或 Session。现有 smoke runner唯一加入真实 Loader场景：Manager read Catalog后对一 available candidate做 admit，对另一做 reject；断言 admit Session独立、native Skill可加载、旧/非 Manager写入拒绝、冷重启读到相同 Definition/ownership及 Archive安全字段。contract test覆盖普通 Participant无目录/决定数组、失败原因只安全 code、Archive不含 Session/配置。
验证：
~~~bash
pnpm --dir plugin exec vitest run --project host tests/unit/domain/meeting-state-v1-validation.spec.ts
...
前置状态：T0–T8 全部 PASS；工作树仅含本切片必要文件。
...
git diff --check
~~~
PASS：四命令退出 0；T0–T8断言仍成立，coverage只记真实执行结果，RUNBOOK删除后无残留相对链接。完整集合选择依据：本切片改变公共 command、权限、Repository/DSH生命周期与恢复，故需全工程 verify和真实 Host smoke；CI门禁仍按 PR Rules另行处理。
...
Audit：**Executable**。T0 的九个固定目标路径当前均不存在，因此当前 checkout 的机械执行结果是按 T0 首个 `test -f` 失败正常 STOP；这是一项已固定的外部代码前置，不再要求执行者或后续作者选择入口。T0 已固定现有 `MeetingRepositoryPort` 的复用边界、全部目标 symbol、必须真实运行的原子 commit/replay/projection/end/archive/recovery 断言，以及 PASS 后直接进入 T1 的规则。上游若使用不同路径或第二 Repository，T0 必须 STOP，不能由执行者适配。

T2/T8 已纳入当前 target validator、dispatcher、fixture 和既有测试，required `identityRecommendations` 与 `identityProvenance` 不允许隐式默认或未经 validator/codec 验证。T5 的 DSH 输入来源仍由既有固定路径提供：`MeetingIdentityApplicationDepsV1.readVerifiedSessionScope` 给出 teamId、Captain parent Agent/Session 与 Manager Session，`dsh/session-adapter.ts::startParticipantSession` 接收已验证 parent、Subagent runtime/provider、由 `role-composition/resolve.ts` 转换的 persona/toolFilter/agentOptions；`dsh-capabilities.ts::validateSharedRoleCapabilities` 与 `dsh/session-adapter.ts::requireContinuableProvider` 只产生固定 CAPABILITY_MISSING/missing=[] 映射。`IdentityAdmissionPortV1` 只适配同一 MeetingRepository ownership 与受控 DSH Session API，不得创建 registry/第二存储或自行选择 provider。

T8 的真实 Host Catalog producer、两个 available candidate 与 required native Skill 是已写明的外部前置门槛；缺失时按 T8 STOP，不把 mock smoke 写成通过。Author/Audit 实际检查：2026-09-16 从仓库根运行 `node .github/scripts/check-doc-links.mjs`，442 个本地文件链接、0 错误（anchors 不在脚本覆盖）；运行 `git diff --check`，退出 0；逐项只读检查 T0 九个路径，九个均缺失，故当前执行 Not Covered 为 T1–T9、真实 DSH Loader、Catalog producer 与 smoke。工作树另有用户已授权的 target transition 去重改动，执行者必须保留，不把它们计入本 RUNBOOK 完成证据。
