...
当前已建立 `.github/workflows/pr-verify.yml`，只校验文档治理结构和 PR diff 格式。应用脚手架尚未建立，因此当前 CI 不执行也不声称执行构建、lint、类型检查、测试或打包。工程初始化后，必须根据真实 workspace 和工具链补充验证矩阵。
...
- 工程脚手架建立后，必须把真实的格式化、lint、类型检查、单元测试、集成测试和打包命令接入本规则。
