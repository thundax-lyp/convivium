...
每个 outbox item 具有稳定 `id`、`deliveryId`、`kind`、JSON payload、attempt count、状态、lease owner、lease token、lease deadline、retry time 和最后错误。claim 在事务内原子取得 lease；completion 必须带回同一 owner/token，过期或旧 lease 的 completion 返回 `LEASE_LOST`，不得覆盖新 worker 的状态。

外部调用不在 SQLite 事务内执行。成功、可重试失败和终止失败都通过独立事务完成；相同 `deliveryId` 重投不得产生重复领域事实。

### Recovery

`recover` 只处理当前 Meeting 数据库：回收过期 lease、返回未完成 outbox、bootstrap 状态和当前 snapshot。workspace 目录扫描、跨 Meeting 隔离和 DSH orphan Session 处理属于上层 `RecoveryCoordinator`，不由 Repository 隐式完成。
...
当前 schema 至少包含：`meetings`、`meeting_events`、`idempotency_receipts` 和 `outbox`。JSON 字段必须带稳定对象结构，由上层 transition 负责领域 schema 校验。

## Error And Permission Semantics

Repository 错误必须使用结构化 `RepositoryError`，至少区分：`MEETING_NOT_FOUND`、`VERSION_CONFLICT`、`IDEMPOTENCY_CONFLICT`、`SQLITE_BUSY`、`SCHEMA_VERSION_UNSUPPORTED`、`CORRUPT_DATABASE`、`LEASE_LOST`、`INVALID_STATE` 和 `CLOSED`。
