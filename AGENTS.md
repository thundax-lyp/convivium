...
- 人工 DSH Web 调试和人工验收以仓库 `dsh-workspace/` 为固定启动工作目录，`DSH_HOME` 固定为 `dsh-workspace/dsh-home/`；项目目录为 `dsh-workspace/projects/meetings-view/`，安装与会议 SQLite 固定在 `dsh-workspace/web-ui/convivium-user/`，不按 Git HEAD 重建。自动 `smoke:profile` 仍使用独立临时环境，不作为人工 Web 入口；路径和操作见 [DSH Smoke](docs/50-operations/HOW-TO-DSH-SMOKE.md#人工-web-调试与验收)。
