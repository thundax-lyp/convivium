...
发行包附带八个 Definition、共享 `convivium` Preset、八个原生 DSH Skills 和显式部署 patch：一个专职 Manager、一个专职 Evidence Reviewer，以及六个 Contributor。资源中不包含 Scribe。
...
Evidence Reviewer `1.2.0` 收到 Runtime 提供的 immutable pending set 后，由 coordinator 为每个版本创建独立的 DSH 原生 one-shot worker；worker 不具备 Meeting command authority。Coordinator 只继承 `skill`、`subagent` 与 `convivium_submit_review_batch`，省略失败、取消或非法结果，并只通过一次 batch command 原子提交有效 reviews。
