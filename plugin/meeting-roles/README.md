...
Evidence Reviewer `1.2.1` 收到 Runtime 提供的 immutable pending set 后，由 coordinator 为每个版本在本轮只创建一个独立的 DSH 原生 one-shot worker，不创建 replacement worker；worker 不具备 Meeting command authority。Coordinator 只继承 `skill`、`subagent` 与 `convivium_submit_review_batch`，省略失败、取消或非法结果；本轮有合法结果时只通过一次 batch command 原子提交，没有合法结果时结束并交由 Runtime outbox 重试。
