...
const moduleBoundaries: readonly ModuleBoundary[] = [
  { name: 'protocol', mayImport: [], forbiddenRuntimeImports: ['node:', 'react', 'sqlite', '@deepseek-ai/dsh-'] },
  { name: 'domain', mayImport: [], forbiddenRuntimeImports: ['node:', 'react', 'sqlite', '@deepseek-ai/dsh-'] },
  { name: 'repository', mayImport: ['domain'], forbiddenRuntimeImports: ['@deepseek-ai/dsh-', 'react', 'http'] },
  { name: 'runtime', mayImport: ['domain', 'repository', 'dsh', 'projection'], forbiddenRuntimeImports: ['node:sqlite', 'react', 'http'] },
  { name: 'dsh', mayImport: ['domain'], forbiddenRuntimeImports: ['repository', 'react'] },
  { name: 'projection', mayImport: ['protocol', 'domain'], forbiddenRuntimeImports: ['@deepseek-ai/dsh-', 'react', 'node:', 'sqlite'] },
  { name: 'tools', mayImport: ['protocol', 'runtime'], forbiddenRuntimeImports: ['repository', 'client'] },
  { name: 'http', mayImport: ['protocol', 'runtime'], forbiddenRuntimeImports: ['repository', 'client'] },
  { name: 'client', mayImport: ['protocol'], forbiddenRuntimeImports: ['node:', 'sqlite', 'repository', 'runtime', 'dsh', 'tools', 'http', '@deepseek-ai/dsh-'] },
]
...
function moduleForFile(file: string): ModuleName | undefined {
  ...
  return moduleBoundaries.some(({ name }) => name === topLevel) ? topLevel as ModuleName : undefined
}
...
function violations(module: ModuleName, specifiers: readonly string[]): string[] {
  ...
  return specifiers.flatMap((specifier) => {
    ...
    if (imported && !boundary.mayImport.includes(imported)) return [`${module} may not import ${imported}`]
...
describe('plugin module boundaries', () => {
  ...
  it('rejects a temporary Client-to-Host import', () => {
    expect(violations('client', ['../runtime/index.js'])).toEqual(['client may not import ../runtime/index.js'])
  })
})
