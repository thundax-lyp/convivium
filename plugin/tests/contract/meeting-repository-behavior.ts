...
type OpenRepository = () => Promise<MeetingRepositoryPort>;

function defineCreationCases(openRepository: OpenRepository): void {
...
function defineCommandCases(openRepository: OpenRepository): void {
...
function defineOwnershipCases(
    ...
    it("maps corrupted persisted state to RepositoryError", async () => {
        await expect(harness.rejectCorruptedReadyState()).rejects.toMatchObject<RepositoryError>({
...
function defineMailAndSessionCases(
    openRepository: OpenRepository,
    openCreatingRepository: OpenRepository
): void {
...
export function defineMeetingRepositoryBehaviorContract(
    ...
    const openRepository: OpenRepository = () => harness.open();
    const openCreatingRepository: OpenRepository = async () => {
        const repository = await openRepository();
        await repository.create({
            requestId: "create",
            authorization,
    ...
    describe(name, () => {
        defineCreationCases(openRepository);
        defineCommandCases(openRepository);
        defineOwnershipCases(openRepository, openCreatingRepository, harness);
        defineMailAndSessionCases(openRepository, openCreatingRepository);
    });
}
