...
`);
    ...
    },
    {
        ...
        apply(db) {
            ...
            if (outbox?.present === 1)
                db.exec("ALTER TABLE outbox ADD COLUMN priority INTEGER NOT NULL DEFAULT 50;");
            db.exec(`
CREATE TABLE meeting_mail (
...
CREATE INDEX meeting_mail_pending ON meeting_mail(meeting_id, recipient_participant_id, status, deadline_at, created_at);
`);
        }
    }
];
