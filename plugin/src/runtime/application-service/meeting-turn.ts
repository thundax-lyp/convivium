...
export { assignTurnAttempt, initializeFirstMeetingTurn } from "./initialize-meeting-turn.js";
...
export function createMeetingTurnApplication(dependencies: MeetingTurnApplicationOptions) {
    ...
    > & {
            ...
            try {
                ...
                const committed = await stored.repository.execute({
                    ...
                    transition: (snapshot) => {
                        ...
                        const submittedTurn = transitionState.currentTurn;
                        const taskOutbox: Array<{
                            deliveryId: string;
                            kind: "dispatch";
                            payload: JsonObject;
                        }> = transitionState.meetingTasks
                            .filter(
                                (task) =>
                                    task.status === "queued" &&
                        ...
                        const turnStatus =
                            ...
                            (transitionState.status === "partial" ? "truncated" : "completed");
                        return {
                            state: transitionState as unknown as JsonObject,
                            result: {
                                messageId,
                                messageSeq: transitionState.messageSeq,
                                turnStatus,
        ...
        async fallbackManagerPlanning(input: ManagerFallbackInput): Promise<void> {
            const state = (await input.repository.read()).state as unknown as MeetingState;
            const planningAttempt = state.manager.currentPlanningAttempt;
            if (planningAttempt?.id !== input.attemptId) return;
            const ids = {
                turnId: `turn-${state.turnSeq + 1}`,
                stepId: (index: number) => `step-turn-${state.turnSeq + 1}-${index}`
            };
            const requestId = `manager-fallback:${input.attemptId}:${input.reasonCode}`;
            const requestHash = serializeValidatedRequestV1({
                attemptId: input.attemptId,
            ...
            await input.repository.execute({
                requestId,
                commandKind: "manager_fallback",
                authorization: {
                    callerBinding: `runtime:${input.meetingId}`,
                    capabilityId: `runtime:${input.meetingId}`,
                    attemptId: input.attemptId
                },
                requestHash,
                expectedMeetingVersion: input.observedMeetingVersion,
                transition: (snapshot) => {
                    const current = snapshot.state as unknown as MeetingState;
                    const transition = failManagerPlanningAndCreateFallback(
                        current,
                        {
                            meetingId: input.meetingId,
                            planningAttemptId: input.attemptId,
                            deliveryId: current.manager.currentPlanningAttempt?.deliveryId ?? "",
                            observedMeetingVersion: input.observedMeetingVersion,
                            dispatchableParticipantIds: current.participants
                                .filter((participant) =>
                                    isParticipantDispatchableNow(current, participant)
                                )
                                .map((participant) => participant.id),
                            now: input.now,
                                ...
                                : [
                                      {
                                          ...
                                          payload: {
                                              role: "participant",
                                              participantId: turn.steps[0]!.attempt!.participantId,
                                              attemptId: turn.steps[0]!.attempt!.attemptId,
                                              turnId: turn.id,
                                              stepId: turn.steps[0]!.id
                                          }
