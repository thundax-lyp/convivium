...
发行包的 `config/` 包含七个 Definition、七份角色 `AGENTS.md`、七个独立 `convivium-*` Preset、五项可复用能力 Skill 和 `cordis.patch.yml`。Definition 当前版本均为 `2.0.0`。AGENTS 描述身份、职责与权限，Skill 描述如何完成一类工作。
...
用户从 `Meetings` 面板创建与控制会议，无须新建 Captain Session 或选择共享 Preset。每个会议身份使用独立、无 parent 的 AgentSession；用户输入 Session 关闭不影响会议投递。正式交流只经 Meeting Runtime。专职 Reviewer 使用 `convivium_run_review_worker` 对每个 immutable version 创建 one-shot subagent，worker 没有 Meeting command authority，只有合法 completed 结果才能提交审核。
