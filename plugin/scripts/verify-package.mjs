...
const packageRoot = resolve(rootFlag >= 0 ? process.argv[rootFlag + 1] : new URL('..', import.meta.url).pathname)
...
const missingArtifacts = requiredArtifacts.filter((path) => !existsSync(resolve(packageRoot, path)))
...
const result = {
  ...
  filesAllowlistIsClosed: JSON.stringify(files) === JSON.stringify(['lib', 'cordis.patch.yml', 'README.md']),
  bundlePatchMatchesPackageName: Boolean(packageName && patch.includes(packageName)),
  clientManifestIsComplete: client?.platform === 'web' && Array.isArray(client.inject) && client.inject.length > 0,
