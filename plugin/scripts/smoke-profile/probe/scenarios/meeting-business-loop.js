...
export async function runMeetingBusinessLoopScenario(runtime) {
    const { ctx, captain, callTargetTool, callTargetToolResult, nextCall, assert, writeResult, messageTexts } = runtime;
    ...
    const definitions = [
        ...
        ["reviewer", "convivium.verification_reviewer", "1.2.0", "evidence_reviewer"],
    ...
    const contributorKeys = ["contributor-a", "contributor-b", "contributor-c", "contributor-d", "contributor-e", "contributor-f"];
    ...
    const roleTurns = Object.fromEntries(keys.map((key) => [key, reviewerTurnSummary(currentAgent(key))]));
    assert(
    ...
    const knownSessionIds = new Set([String(captain.agent.id), ...Object.values(agents).map((agent) => String(agent.id))]);
    ...
    for (const key of ["contributor-a", "contributor-b"]) {
        ...
        const result = await callTargetTool(ctx, contributor, "convivium_raise_hand", { protocolVersion: 1, meetingId, expectedMeetingVersion: version, requestId: `loop-raise-${key}`, action: { kind: "raise_hand", roundId, purpose: "submit smoke evidence" } }, nextCall());
        raised.push(key);
    ...
    for (const [index, key] of raised.entries()) {
        ...
        try {
            const contributor = await runtime.waitForAgent(ctx, agents[key].id);
            await callTargetTool(ctx, contributor, "convivium_submit_evidence", { protocolVersion: 1, meetingId, expectedMeetingVersion: beforeEvidence.version, requestId: `loop-evidence-${key}`, action: { kind: "submit_evidence", contributionId: contributions[index], evidence: evidence(key) } }, nextCall());
        } catch (error) {
            throw new Error(`submit evidence failed for ${key} contribution=${contributions[index]} version=${beforeEvidence.version} rounds=${JSON.stringify(beforeEvidence.rounds)}: ${error.message}`, { cause: error });
        }
    }
    ...
    for (let attempt = 0; attempt < 120; attempt += 1) {
        if (runtime.observedMessages(currentReviewer()).flatMap(messageTexts).some((text) => text.includes('"pending"'))) break;
        await new Promise((resolve) => setTimeout(resolve, 200));
    }
    assert(runtime.observedMessages(currentReviewer()).flatMap(messageTexts).some((text) => text.includes('"pending"')), "review request was not delivered to the coordinator");
    let workers = [];
    for (let attempt = 0; attempt < 300; attempt += 1) {
        workers = runtime.observedAgents().filter((agent) => !knownSessionIds.has(String(agent.id)) && ctx.agents.get(agent.id) === agent);
        if (workers.length >= 2) break;
        await new Promise((resolve) => setTimeout(resolve, 200));
    }
    assert(workers.length >= 2, `reviewer did not create two live one-shot workers; observed=${JSON.stringify(runtime.observedAgents().map((agent) => String(agent.id)))}; turns=${JSON.stringify(reviewerTurnSummary(currentReviewer()))}; tools=${JSON.stringify(reviewerToolSummary(currentReviewer()))}`);
    const authorityVersion = (await read()).version;
    for (const [index, worker] of workers.slice(0, 2).entries()) {
        const unauthorized = await callTargetToolResult(ctx, worker, "convivium_raise_hand", { protocolVersion: 1, meetingId, expectedMeetingVersion: authorityVersion, requestId: `loop-worker-authority-${index}`, action: { kind: "raise_hand", roundId, purpose: "must be rejected" } }, nextCall(), { retryUnknown: false });
        const toolUnavailable = unauthorized.isError && String(unauthorized.error?.message).includes("unknown tool");
        const commandRejected = !unauthorized.isError && unauthorized.value?.kind === "rejected" && unauthorized.value.error?.code === "UNAUTHORIZED";
        assert(toolUnavailable || commandRejected, "review worker gained Meeting command authority");
    }
    let reviewedView;
    for (let attempt = 0; attempt < 900; attempt += 1) {
        reviewedView = await read();
        const reviewedVersionIds = reviewedView.evidenceReviews?.map((review) => review.versionId) ?? [];
        if (versionIds.every((versionId) => reviewedVersionIds.includes(versionId))) break;
        await new Promise((resolve) => setTimeout(resolve, 200));
    }
    assert(versionIds.every((versionId) => reviewedView?.evidenceReviews?.some((review) => review.versionId === versionId)), `reviewer did not atomically submit the worker review batch; currentVersion=${reviewedView?.version}; reviewed=${JSON.stringify(reviewedView?.evidenceReviews?.map((review) => review.versionId) ?? [])}; turns=${JSON.stringify(reviewerTurnSummary(currentReviewer()))}; tools=${JSON.stringify(reviewerToolSummary(currentReviewer()))}`);
    for (const key of raised) {
        const turns = reviewerTurnSummary(currentAgent(key));
        if (turns.starts > turns.ends)
            ctx.subagents.interrupt(agents[key].id, { kind: "ancestor", agent: captain.agent });
    }
    let reviewDelivered;
    for (let attempt = 0; attempt < 300; attempt += 1) {
    ...
    assert(reviewDelivered?.reviewDeliveries?.length === versionIds.length && reviewDelivered.reviewDeliveries.every((delivery) => delivery.status === "sent"), `evidence reviews were not delivered: ${JSON.stringify({ version: reviewDelivered?.version, reviews: reviewDelivered?.evidenceReviews?.map((review) => review.versionId), deliveries: reviewDelivered?.reviewDeliveries })}`);
    ...
    try {
        ...
        throw new Error(`publish failed: ${error.message}; state=${JSON.stringify({ version: reviewDelivered.version, round: reviewDelivered.rounds.find((round) => round.id === roundId), contributions: reviewDelivered.contributions, reviews: reviewDelivered.reviews, reviewDeliveries: reviewDelivered.reviewDeliveries })}`, { cause: error });
    }
    ...
    const startedNoticeCounts = Object.fromEntries(contributorKeys.map((key) => [key, new Set(runtime.observedMessages(currentAgent(key)).flatMap(messageTexts).filter((text) => text.includes("meeting_started"))).size]));
    assert(Object.values(startedNoticeCounts).every((count) => count === 1), `meeting_started notice counts are invalid: ${JSON.stringify(startedNoticeCounts)}`);
    await writeResult({ ok: true, scenario: "meeting-business-loop", meetingId, assertions: ["target-create", "meeting-started", "two-evidence", "review-batch", "worker-authority", "published", "archived"], observed: { status: archived.lifecycle.status, evidenceVersionIds: versionIds, startedNoticeCounts, workerSessionIds: workers.slice(0, 2).map((worker) => String(worker.id)) } });
}
