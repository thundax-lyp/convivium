...
以下 FR-10 任务按 T0 → T8 顺序执行，前一步 PASS 才能开始下一步。2026-09-07 用户要求依据 RUNBOOK 制定清单；产品实现执行许可尚待确认。允许文件、命令、PASS/STOP 和恢复方式以 RUNBOOK 对应步骤为准，不在此复制方案。

- [ ] `FR-10 / T0 / 执行基线`：核对实现前置条件
    - 依据文档：[RUNBOOK T0](docs/30-designs/RUNBOOK-FR10-REFERENCED-MINUTES.md#t0锁定前置工作边界)
    - 确认依据：2026-09-07 清单编制请求；执行待确认。
    - 处理动作：获得执行许可后，核对独立分支、目标文件基线、DSH 版本并运行基线验证。
    - 验收点：T0 和第 7.1 节基线门禁 PASS；现有指定用例通过，新增 T6 用例记待执行；冲突或失败即 STOP。
...
- [ ] `FR-10 / T8 / 验证与收口`：完成整项验收并迁移证据
    ...
    - 确认依据：2026-09-07 清单编制请求；执行待确认，commit/push/PR/merge 不在本清单授权内。
