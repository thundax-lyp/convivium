...
- [ ] `B-01 / DSH TeamTask API`：取证闭环 B 所需的真实 DSH 能力
    - 依据文档：`docs/30-designs/RUNBOOK-TEAMTASK-HAND-RAISE.md` 第 4.3、8 节
    - 确认依据：RUNBOOK 已完成 A/B 双向审核；待用户确认执行
    - 处理动作：验证并记录 TeamTask create/associate、correlation、授权读取、terminal observation、Participant Session 通知和串行约束；任一必需能力缺失时记录 `UNSUPPORTED_CAPABILITY` 并停止。
    - 验收点：`docs/40-readiness/TEAMTASK-HAND-RAISE-EVIDENCE.md` 可追溯到锁定依赖的包、版本、export、类型和实现源码，且包含 `pnpm verify:environment`、`pnpm verify:contract`、`pnpm list --depth 0` 的结果。

- [ ] `B-02 / TeamTask 与 HandRaise 契约`：补齐最小协议、领域模型和 schema
    - 依据文档：`docs/30-designs/RUNBOOK-TEAMTASK-HAND-RAISE.md` 第 4.2、4.4、5/T1 节
    - 确认依据：`B-01` 通过后执行；待用户确认执行
...
- [ ] `B-07 / 验证与收口`：验证闭环 B 并迁移临时结论
    ...
    - 确认依据：`B-06` 通过且闭环 A 已有公开 commit/PR diff 后执行；待用户确认执行
