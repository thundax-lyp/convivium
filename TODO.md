...
- [ ] `会议创建`：切换新版本贡献模型（T7）
    ...
    - 确认依据：2026-09-14 本对话确认方案并要求登记；2026-09-14 本对话明确授权依序执行、删除并提交。
    - 处理动作：启用 reviewer 前置校验和贡献启动流程，接好恢复标记；按 2026-09-15 确认取消旧版本兼容，不新增 legacy 分支或历史测试夹具。
    - 验收点：非法新建零副作用；无 Turn 事件；本版本合法创建请求幂等重放；不执行、迁移或删除旧记录；当前权限、恢复与归档约束通过。
