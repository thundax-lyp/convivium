...
- [ ] `MAD-10 / 部署验证与 readiness`：执行完整门禁并保存真实证据
    ...
    - 修复授权：2026-09-08 用户追加授权解决部署断点并完成全部 TODO；先执行 RUNBOOK T4b 的原生资源定位/config 组合修复与回归，单独提交，再从完整 verify 重跑本项。历史失败证据保留。2026-09-08 用户明确豁免本轮 web_fetch；先增加默认严格、显式跳过且结果标注 Not Covered 的探针开关并单独提交，再执行四条门禁。三研究搜索、九角色、权限和冷恢复仍必过；不再要求修改 DNS。
    ...
    - 验收点：T5 四条固定命令 exit 0、所有 Restore PASS，随后 V-DOC/diff check 通过；九角色加载、三类研究搜索、权限拒绝、模型差异和冷恢复均有本次证据；首次失败 STOP，仅 web_fetch 按本次用户豁免记为 Not Covered，其余必过项不得豁免。
