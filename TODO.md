...
首发范围已确认；当前仅授权文档工作，以下实施任务仍待执行许可。MAD 编号用于任务依赖，RUNBOOK 的 T1–T6 仍是阶段门禁；阶段内拆分不新增范围，也不允许以部分任务完成代替整个阶段 PASS。T0 已确认，不列待办。每项相关文件均为仓库相对路径；计划新增路径不表示文件已存在。

- [ ] `MAD-01 / 正式契约`：复核首发契约一致性
    - 依据文档：[需求](docs/10-requirements/MEETING-ORCHESTRATION-REQUIREMENTS.md) FR-14、BR-11、验收 35–40；[接口](docs/20-interfaces/MEETING-AGENT-DEFINITION-INTERFACE.md) Data And State Contract、Compatibility；[架构](docs/00-governance/ARCHITECTURE.md) Confirmed Baseline、DSH Agent Sessions；[RUNBOOK](docs/30-designs/RUNBOOK-MEETING-AGENT-ROLE-DESCRIPTION.md) T1。
    - 确认依据：2026-09-08 本任务对话确认初发最终模型、无迁移和九角色完整部署；本轮仅确认 TODO 细化，未授权实施。
    - 前置：实施授权后首先复核；T0 已确认，不重复登记或运行。
    - 相关文件：只读核对，冲突修正限于 `docs/00-governance/ARCHITECTURE.md`、`docs/10-requirements/MEETING-ORCHESTRATION-REQUIREMENTS.md`、`docs/20-interfaces/MEETING-AGENT-DEFINITION-INTERFACE.md`、`docs/30-designs/ROLE-COMPOSITION-DESIGN.md`、`docs/30-designs/MEETING-ORCHESTRATION-DESIGN.md`（4.4）、`docs/30-designs/CONVIVIUM-IMPLEMENTATION-DESIGN.md`（旧样本定位）。
    - 处理动作：核对已编写的字段、能力所有权、初发格式和部署责任；相同内容不重复改写。
    - 验收点：T1 的字段检索、V-DOC、git diff --check 通过；六份正式文档一致，DSH 原生 persona/agentOptions 保留，未决冲突必须 STOP。
