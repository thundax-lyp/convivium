...
用稳定的 lowercase kebab-case string literal 定义 identity，由 Settings API 的类型和运行时校验 namespace，并让 Host 与 Browser 使用完全相同的 namespace 值。Browser 可以重复该字符串拼写；不要为了共享常量而跨插件 value-import Host 模块。已有 cordis entry 的 Consumer 在 `ctx.inject(['settings'], settingsCtx => ...)` 内使用 `settingsCtx.settings.installSection(ownerCtx, namespace, schema, entry, hooks)`，把 entry config 作为 base，同时在没有 Settings Provider 的组合中仍使用 composition value。Schema 表达字段规则；跨字段或外部可达性等 schema 不能表达的约束放进 registration `validate`，使无效写入在 commit 前失败。
...
`installSection` 的 `ownerCtx` 是配置 Consumer 的 context，不能错误换成 Settings Provider 的 context。无 Settings 时 Consumer 先使用 validated entry；Provider 存在时通过 setSource/watch 接管，卸载后恢复 entry，避免 teardown 重新注册资源。

Host Remote 面由 `settingsController` 提供，统一返回 redacted namespace view，并区分 `settings/conflict` 与 `settings/rejected`。Client settings scope 由 `ui-settings` 包拥有，不再从旧 Client runtime contract 导入。
...
Settings controller 同时承担面向 UI 的凭证操作，但 Settings 的 redacted namespace 文档不等于 credential record。凭证 reference、safe record summary、authorization descriptor/attempt 与 cancel 的区别见 [凭证与账号授权](credentials-authorization.md)。Secret 不通过普通 namespace snapshot 或授权进度事件回传；取消 attempt 不承诺撤销已经发生的外部授权或阻止不合作 flow 的迟到提交。
