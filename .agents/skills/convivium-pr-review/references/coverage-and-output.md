...
* 问题：具体错误及根因。
* 触发：能够从代码证明的场景或调用路径。
* 当前影响：说明属于 runtime、正式契约、验证结果还是仅未来设计，并给出用户、数据、权限、性能或维护后果。
* 最小修复：能够消除上述触发条件并保持必要边界的最小改动。
* 范围上限：本 finding 不要求实现、重构或引入的相邻能力。
* 证据：需求、接口、设计、测试或命令。
```

`最小修复` 只定义修复边界，不替作者设计完整实现。若删除或收窄即可消除触发条件，不得建议新增机制。`范围上限` 必须点名最容易被顺带扩张的相邻能力；没有相邻扩张风险时写 `无`。
