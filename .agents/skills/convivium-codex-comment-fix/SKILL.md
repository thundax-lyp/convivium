...
description: Explicitly invoked workflow for processing unresolved Codex review threads and actionable Codex review-summary findings on a Convivium pull request, applying minimal safe fixes, and recording each disposition through the available GitHub reply channel. Accepts #5 or 5; when omitted, selects the open PR for the current branch.
---

# Convivium Codex Comment Fix

只有用户显式调用 `$convivium-codex-comment-fix` 时，才处理指定 PR 的未关闭 Codex review threads 与尚未处置的 Codex review-summary findings；参数可为 `#5`、`5`，或省略以解析当前分支 PR。单独说“Codex 有评论”不应自动触发该 skill。一次明确调用即授权在本任务范围内进行 reaction、代码修改、验证、commit、回复和可用时的 resolve。
...
- 处理该 PR 中未解决的 Codex review threads，以及 Codex 写在 PR review summary 正文中、没有对应 review comment/thread 且尚未处置的 actionable finding。普通 issue comments 不是 finding 来源，唯独“冗余额度提示”和本 skill 为 summary finding 写入的处置回复例外。
- 目标终点是：thread finding 已分类并完成对应 reaction；已接受 finding 已修复、验证和回复，明确拒绝的 finding 已说明依据并回复，真实但暂缓或需要决策的 finding 已说明状态。thread finding 按原规则 resolve 或保持 unresolved；summary finding 通过带稳定标记的 PR issue comment 记录处置，因为 GitHub 不为 review summary 提供逐 finding reaction、reply 或 resolve。仅报告审查额度不足的冗余 Codex 评论直接删除。代码修复必须形成 commit。

## Review Summary Finding

Codex 可能不再创建独立 review comments，而是把多个 finding 放进一个 `COMMENTED` review 的 summary body。GitHub 页面会把其中的 blob URL 渲染为“文件名、commit、行号与代码表格”；REST API 返回的原始 body 通常是以下重复结构：
...
- 读取 PR reviews 时，只在作者已确认为 Codex、review body 含具体 blob URL/文件行范围及可执行技术内容时识别 summary finding。GitHub 页面复制文本中的文件链接、`Lines ... in <commit>`、代码表格和 `&#x20;`/`&#xA;` 等渲染实体是同一格式，不要求 API raw body 逐字包含这些展示元素。
- 按 raw review body 中 actionable finding 的出现顺序赋予稳定来源键 `review:<review-id>:finding:<1-based-index>`。末尾 “About Codex in GitHub” 说明、普通 review 引导语和纯额度提示不计入 finding。
- 同一内容如果已有 review comment/thread，以 thread 为唯一来源，不再把 summary 副本作为第二条 finding。通过 review ID、文件/行范围、标题和正文核对，而不是只凭标题相似。
- summary finding 没有逐项 reaction API，也没有可 resolve 的 thread；其 reaction 与 resolve 均记为 `Not Applicable`，不得伪造 reaction、创建空 thread 或声称已 resolved。
- 对同一 review 的 summary findings 使用一条汇总 PR issue comment 回复，逐项列出来源键、文件/行、分类、依据、fix PR/commit 和验证结果。回复必须包含下列稳定标记，每个 finding 一个；`disposition` 使用 `fixed|rejected|deferred`：

```html
<!-- convivium-codex-summary review=5244237476 finding=1 disposition=fixed -->
```

- 运行前读取现有 issue comments 中的标记：最新的 `fixed|rejected` 视为已处置，不重复回复；最新为 `deferred` 且本轮仍无实际 fix PR/commit 时不重复回复，只报告仍暂缓。后续形成修复后可另发带同一 review/finding 和 `disposition=fixed` 的回复，以最新标记为准。
- summary 回复成功后回读 issue comment，确认正文、全部标记和实际 comment ID；命令成功但未回读不算完成。
...
- GitHub review thread 与 review-summary finding 的 PR issue comment 回复使用中文。
...
- 后续修复 commit 必须在 commit body 写明来源。thread 使用 `Refs: PR #N, Codex comment #<comment-id>`；summary finding 使用 `Refs: PR #N, Codex review #<review-id>, finding #<index>`；多个来源逐条列出。
- 后续修复 commit 和 PR 产生后，thread 必须回到原评论补充实际 `fix PR` 与 `fix commit`，再 resolve；summary finding 必须发布同一来源键、`disposition=fixed` 的新回复并回读确认。
...
1. 读取当前分支、工作区、PR head/base、未解决 threads、PR review summaries、review comments 和 issue comments；从 threads 与符合“Review Summary Finding”的 summary body 建立去重后的 finding 集合，并用已有 summary 回复标记排除已处置项。确认 PR 属于当前仓库、状态为 open 或 merged、base 是 `main`。已关闭且未合并的 PR 必须停止。在确认 PR 存在且满足状态条件前不得添加 reaction、修改代码、commit 或 push。指定其他 PR 时，`comment PR` 的 head 可以不是当前分支，但不得把当前分支的 commit 冒充为该 PR 的修复。
...
3. 先按“冗余额度提示”规则删除可确认的额度提示，并重新读取对应来源、threads 和 summaries；再对每条剩余 Codex finding 建立触发条件、影响、代码证据和正式依据，按“Finding 与建议方案分离”分类：
   - `accept-as-proposed` 或 `accept-with-smaller-fix`：thread finding 先对原评论添加 👍；summary finding 记录 reaction `Not Applicable`。随后实现最小安全修复和回归测试。
   - `reject`：thread finding 添加 👎；summary finding 记录 reaction `Not Applicable`。不修改代码，并在对应回复中说明 finding 不成立或明确超出当前范围的具体依据。
   - `defer-or-decision`：thread finding 添加 👎；summary finding 记录 reaction `Not Applicable`。不修改代码，回复 blocker、缺失决策或 follow-up 条件；thread 保持 unresolved，summary 标记为 `deferred`。
...
7. 回复并回读：thread 只有 reply 成功后才 resolve；summary finding 按“Review Summary Finding”写入并回读带标记的汇总 issue comment，不能 resolve。接受的 actionable finding 回复必须包含处理结论、`comment PR`、`fix PR`、`fix commit` 和实际验证结果；不采纳 finding 的回复必须包含 `comment PR` 和具体依据；暂缓 finding 只能说明 follow-up 状态。`accept-with-smaller-fix` 的回复还必须分别说明原 finding 的触发条件、未采用建议方案的原因，以及更小修复如何消除同一触发条件并保持必要边界。
8. 推送后重新读取 PR threads、review summaries、summary 回复标记和 checks，处理本轮由修复引起的新 Codex findings；达到停止条件后结束。
...
- 没有未关闭 Codex threads 且没有未处置 summary findings 时不创建空 commit；仅报告 PR 已无待处理评论。不得仅因 `reviewThreads.totalCount === 0` 忽略含 actionable finding 的 Codex review summary。
...
- review summary finding 的回复使用 PR issue comment；它是 summary finding 的回复载体，不把其他普通 issue comments 纳入 finding 范围。GitHub 未提供 summary 内逐 finding reaction/resolve 时必须如实记录 `Not Applicable`。
...
```text
Refs: PR #87, Codex review #5244237476, finding #1
```
...
汇报 PR、分支、删除的额度提示、处理的 threads 与 summary findings、reaction 或 `Not Applicable`、summary 回复 comment ID、commit、push、验证、CI 状态和剩余风险。明确说明没有修改、没有提交、没有推送、未 resolve 或 summary 无 resolve 能力的原因。
