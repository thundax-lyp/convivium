...
测试部分按 [Test Rules](../../../docs/00-governance/TEST-RULES.md) 检查有效性与资产成本。不自动调用 testcase-review；只有用户显式调用该 Skill 时才组合使用，并保留本次模块范围、只读边界和输出格式。
...
复杂度 finding 必须指出新增或现存机制的当前成本、缺失的 caller/requirement，以及保持当前行为和真实保护的直接替代方案；同时说明删除与保留各自的风险。单一消费者、单一实现、文件数量、代码行数、个人风格或“看起来复杂”不能单独达到 finding 门槛。复杂状态机若由当前并发、恢复、安全或兼容要求证明，不属于过度设计。必要的安全、授权、事务、持久化、DSH adapter 和生命周期边界即使只有一个消费者，也不能仅因“抽象较多”报告为缺陷。
